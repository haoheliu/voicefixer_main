...
class UNetResComplex_100Mb(nn.Module):
    def __init__(self, channels, cond_embedding_dim, nsrc=1):
        ...
        self.encoder_block1 = EncoderBlockRes2BCond(in_channels=channels * nsrc, out_channels=32,
                                                 downsample=(2, 2), activation=activation, momentum=momentum, cond_embedding_dim = cond_embedding_dim)
        ...
        self.conv_block7 = ConvBlockResCond(in_channels=384, out_channels=384,
                                        kernel_size=(3, 3), activation=activation, momentum=momentum, cond_embedding_dim = cond_embedding_dim)
        self.decoder_block1 = DecoderBlockRes2BCond(in_channels=384, out_channels=384,
                                                 stride=(2, 2), activation=activation, momentum=momentum, cond_embedding_dim = cond_embedding_dim)
    ...
    def forward(self, sp, cond_vec):
        ...
        # UNet
        print("input", x.size(), cond_vec.size())
        (x1_pool, x1) = self.encoder_block1(x, cond_vec)  # x1_pool: (bs, 32, T / 2, F / 2)
        print("input", x1_pool.size(), cond_vec.size())
        ...
        (x4_pool, x4) = self.encoder_block4(x3_pool, cond_vec)  # x4_pool: (bs, 256, T / 16, F / 16)
        (x5_pool, x5) = self.encoder_block5(x4_pool, cond_vec)  # x5_pool: (bs, 512, T / 32, F / 32)
        (x6_pool, x6) = self.encoder_block6(x5_pool, cond_vec)  # x6_pool: (bs, 1024, T / 64, F / 64)
        x_center = self.conv_block7(x6_pool, cond_vec)  # (bs, 2048, T / 64, F / 64)
        x7 = self.decoder_block1(x_center, x6, cond_vec)  # (bs, 1024, T / 32, F / 32)
        x8 = self.decoder_block2(x7, x5, cond_vec)  # (bs, 512, T / 16, F / 16)
...
if __name__ == "__main__":
    model = UNetResComplex_100Mb(channels=1, cond_embedding_dim=16)
    cond_vec = torch.randn((1, 16))
    print(model(torch.randn((1,1,101,128)), cond_vec).size())
