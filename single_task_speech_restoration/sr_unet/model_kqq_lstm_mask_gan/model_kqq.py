...
class UNetResComplex_100Mb(nn.Module):
    def __init__(self, channels, nsrc=1):
        ...
        self.after_conv2 = nn.Conv2d(in_channels=32, out_channels=3,
    ...
    def forward(self, sp, wav):
        ...
        (_, cos0, sin0) = magphase(real0, imag0)
        cos0_out = cos_in[:, 0:1, :, :] * cos0 - sin_in[:, 0:1, :, :] * sin0
        sin0_out = sin_in[:, 0:1, :, :] * cos0 + cos_in[:, 0:1, :, :] * sin0
        length = wav.shape[2]
        wav_out = self.f_helper.istft(mag0 * cos0_out, mag0 * sin0_out, length)
