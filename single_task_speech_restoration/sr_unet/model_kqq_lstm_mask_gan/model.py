...
from single_task_speech_restoration.sr_unet.model_kqq_lstm_mask_gan.model_kqq import UNetResComplex_100Mb
...
class DNN(pl.LightningModule):
    ...
    def preprocess(self, batch, train=False, cutoff=None):
        if(train):
            ...
            for i in range(vocal.size()[0]):
                vocal[i,...], LR[i,...], noise[i,...], _snr, _scale = add_noise_and_scale_with_HQ(vocal[i,...],LR[i,...], noise[i,...], snr_l=110,snr_h=110, scale_lower=0.6, scale_upper=1.0)
    ...
    def training_step(self, batch, batch_nb):
        ...
        self.vocal, self.LR, _ = self.preprocess(batch, train=True)

        # for i in range(self.vocal.size()[0]):
        #     save_wave(tensor2numpy(self.vocal[i, ...]), str(i) + "vocal" + ".wav", sample_rate=44100)
        #     save_wave(tensor2numpy(self.LR[i, ...]), str(i) + "vocal_LR" + ".wav", sample_rate=44100)
        ...
        self.generated = self(self.sp_LR_target, self.LR)
