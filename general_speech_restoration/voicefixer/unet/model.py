...
from general_speech_restoration.voicefixer.unet.model_kqq import UNetResComplex_100Mb
...
class Generator(nn.Module):
    def __init__(self,n_mel,hidden,channels):
        ...
        # self.unet = nn.Conv2d(in_channels=channels,out_channels=1,kernel_size=(1,1))
...
class ResUNet(pl.LightningModule):
    ...
    #     _, _, LR, fname = self.preprocess(batch, train=False)
    #
    #     sp, mel_LR_target = self.pre(LR)
    #
    #     output = self(sp, mel_LR_target)
    #
    #     self.log("val_loss",value=loss,logger=True,on_epoch=True,sync_dist=True)
    #     self.draw_and_save(output['mel'],path=os.path.join(self.val_result_save_dir_step, str(fname) + "_LR.png"))
    #     self.draw_and_save(output['orig'], path=os.path.join(self.val_result_save_dir_step, str(fname) + ".png"))
    #     self.draw_and_save(output['mask'], path=os.path.join(self.val_result_save_dir_step, str(fname) + "_mask.png"))
    #     self.draw_and_save(output['addition'], path=os.path.join(self.val_result_save_dir_step, str(fname) + "_addition.png"))
    #     self.draw_and_save(addition, path=os.path.join(self.val_result_save_dir_step, str(fname) + "_addition_orig.png"))
    #     self.draw_and_save(mel_target, path=os.path.join(self.val_result_save_dir_step, str(fname) + "_target.png"))
    #
    #     return {"loss":loss}
